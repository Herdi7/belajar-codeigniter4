...
<?= $this->section('content'); ?>
    ...
    <div class="container">
        <form action="/Anime/save" method="post" enctype="multipart/form-data">
            <?= csrf_field(); ?>
            <input class="mt-3 w-100 d-block form-control <?= ($validation->hasError('judul')) ? 'is-invalid' : ''; ?>" type="text" placeholder="Judul" name="judul" value="<?= old('judul'); ?>">
            <div class="invalid-feedback">
                <?= $validation->getError('judul'); ?>
            </div>
            <input class="mt-3 w-100 d-block form-control" type="text" placeholder="Studio" name="produser" value="<?= old('produser'); ?>">
